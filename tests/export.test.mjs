...
function parseStoredZip(bytes) {
  ...
  assert.equal(centralOffset + centralSize, endOffset, 'central directory must end at EOCD');

  const decoder = new TextDecoder();
  const entries = [];
  let cursor = centralOffset;
  for (let index = 0; index < entryCount; index += 1) {
    assert.equal(readUint32(bytes, cursor), 0x02014B50, 'central directory signature');
    ...
    assert.equal(flags, 0x0800, `${name} must declare UTF-8 names without a data descriptor`);
    assert.equal(compressedSize, uncompressedSize, `${name} must not be compressed`);

    assert.equal(readUint32(bytes, localOffset), 0x04034B50, `${name} local header signature`);
    ...
    assert.equal(readUint32(bytes, localOffset + 14), crc, `${name} local CRC must match central CRC`);
    assert.equal(readUint32(bytes, localOffset + 18), compressedSize, `${name} local size`);
    assert.equal(readUint32(bytes, localOffset + 22), uncompressedSize, `${name} local size`);
    entries.push({ name, data, crc, localOffset });

    cursor += 46 + nameLength + extraLength + commentLength;
  }
  assert.equal(cursor, endOffset, 'central directory must have exactly the advertised length');
  return entries;
}
...
test('buildZip emits deterministic UTF-8 stored entries with valid CRCs and offsets', () => {
  const logo = Uint8Array.from([0x89, 0x50, 0x4E, 0x47, 0x0D, 0x0A, 0x1A, 0x0A]);
  const files = new Map([
    ['main.tex', '\\documentclass{beamer}\n中文内容\n'],
    ['assets/logo.png', logo],
  ]);
  const archive = buildZip(files);
  assert.deepEqual(archive, buildZip(files), 'same input should produce byte-for-byte deterministic output');

  const entries = parseStoredZip(archive);
  assert.deepEqual(entries.map((entry) => entry.name), ['main.tex', 'assets/logo.png']);
  assert.equal(new TextDecoder().decode(entries[0].data), '\\documentclass{beamer}\n中文内容\n');
  assert.deepEqual(entries[1].data, logo);
  for (const entry of entries) {
    assert.equal(entry.crc, referenceCrc32(entry.data), `${entry.name} CRC must match an independent implementation`);
  }

  assert.throws(() => buildZip({ '../outside.txt': 'x' }), /Invalid ZIP entry name/);
  assert.throws(() => buildZip({ '/absolute.txt': 'x' }), /Invalid ZIP entry name/);
  assert.throws(() => buildZip([['same.txt', 'a'], ['same.txt', 'b']]), /Duplicate ZIP entry/);
});
...
test('escapeLatex protects syntax and control characters, including generated TeX source', () => {
  ...
  assert.match(tex, /\\title\{Deck \\textbackslash\{\}input\\\{evil\\\} \\& 50\\%\}/);
  assert.ok(tex.includes('\\brandcoverwithmeta{Cover \\#\\_\\{\\}}{Sub \\$}{Jane \\#}'));
  assert.match(tex, /\\item item\\_1/);
  assert.match(tex, /\\textbf\{H\\&\}: B\\%/);
  assert.match(tex, /\\transduration\{12\.34\}/);
  assert.doesNotMatch(tex, /\\input\{evil\}/, 'user text must not become an executable TeX command');
...
test('buildTex renders sparse bullet layouts without empty itemize environments', () => {
  ...
  const emptyFrame = tex.match(/\\begin\{frame\}\{Empty\}[\s\S]*?\\end\{frame\}/)?.[0];
  const oneColumnFrame = tex.match(/\\begin\{frame\}\{One\}[\s\S]*?\\end\{frame\}/)?.[0];
...
test('parseLogoDataUrl accepts signed PNG/JPEG bytes and rejects unsafe or malformed data', () => {
  ...
  assert.deepEqual(parsedPng, { bytes: png, filename: 'logo.png', mime: 'image/png' });

  const parsedJpeg = parseLogoDataUrl(` data:image/JPEG;base64,\n ${jpegPayload.slice(0, 4)}\n${jpegPayload.slice(4)} `);
  assert.deepEqual(parsedJpeg, { bytes: jpeg, filename: 'logo.jpg', mime: 'image/jpeg' });
  assert.equal(parseLogoDataUrl(null), null);
  assert.equal(parseLogoDataUrl(''), null);

  assert.throws(() => parseLogoDataUrl(`data:image/jpeg;base64,${pngPayload}`), /do not match/);
  assert.throws(() => parseLogoDataUrl('data:image/gif;base64,AAAA'), /PNG or JPEG/);
  assert.throws(() => parseLogoDataUrl('data:image/png;base64,%%%%'), /not valid base64/);
  assert.throws(() => parseLogoDataUrl(`data:image/png;base64,${pngPayload}`, 7), /size limit/);
  assert.throws(() => parseLogoDataUrl(`data:image/png;base64,${Buffer.from([0]).toString('base64')}`), /do not match/);
...
test('downloadBeamer assembles the theme, license, source, and validated logo into a ZIP', async () => {
  ...
  try {
    ...
    const blob = await downloadBeamer(
      { title: '导出测试', slides: [{ type: 'cover', title: '标题', subtitle: '', notes: '', seconds: 30 }] },
      { institution: '机构', department: '', presenter: '演示者', color: '#123456', logo: logoDataUrl },
    );
    assert.equal(blob.type, 'application/zip');
    const entries = parseStoredZip(new Uint8Array(await blob.arrayBuffer()));
    const byName = new Map(entries.map((entry) => [entry.name, entry.data]));
    assert.deepEqual([...byName.keys()], ['main.tex', 'beamerthemeBrand.sty', 'README.md', 'LICENSE', 'logo.png']);
    assert.match(new TextDecoder().decode(byName.get('main.tex')), /\\brandsetlogo\{logo\.png\}/);
    assert.equal(new TextDecoder().decode(byName.get('beamerthemeBrand.sty')), '% test theme');
    assert.equal(new TextDecoder().decode(byName.get('LICENSE')), 'MIT test license');
