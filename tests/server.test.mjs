...
async function withFixture(callback) {
  ...
  await writeFile(join(publicDir, 'index.html'), '<!doctype html><title>BrandBeamer</title>');
  await writeFile(join(publicDir, 'nested', 'asset.txt'), 'safe asset');
  await writeFile(join(root, '.env'), 'AI_API_KEY=should-not-be-served\n');
  await writeFile(join(templateDir, 'beamer', 'beamerthemeBrand.sty'), '% BrandBeamer theme');
...
const sampleGenerate = {
  material: '学校正在推进绿色校园计划。项目先从能源审计开始，再逐步优化照明和空调。',
  title: '绿色校园计划',
  duration: 3,
  brand: {
    institution: '示例大学',
...
test('health, deterministic demo generation, validation, and missing AI configuration', async () => {
  await withFixture(async ({ root, publicDir, templateDir }) => {
    const app = createServer({ rootDir: root, publicDir, templateDir, env: { AI_API_KEY: '', AI_MODEL: 'test-model' } });
    const base = await listen(app);
    try {
      const health = await fetch(`${base}/api/health`);
      assert.equal(health.status, 200);
      assert.deepEqual(await health.json(), { aiConfigured: false, model: 'test-model', demoAvailable: true });
      ...
      assert.deepEqual(first.body, second.body, 'demo output should be deterministic');
      assert.equal(first.body.mode, 'demo');
      assert.equal(first.body.deck.slides.length, 6);
      assert.equal(first.body.deck.slides.reduce((sum, slide) => sum + slide.seconds, 0), 180);
      assert.match(first.body.deck.slides[0].subtitle, /基于所提交材料整理/);
      for (const slide of first.body.deck.slides) {
        assert.ok(['cover', 'bullets', 'columns', 'closing'].includes(slide.type));
      ...
      const materialTooLong = await jsonFetch(base, '/api/generate', {
        method: 'POST',
        body: JSON.stringify({ ...sampleGenerate, material: 'm'.repeat(12_001), mode: 'demo' }),
      });
      ...
      const titleTooLong = await jsonFetch(base, '/api/generate', {
        method: 'POST',
        body: JSON.stringify({ ...sampleGenerate, title: 't'.repeat(81), mode: 'demo' }),
      });
      ...
      assert.doesNotMatch(JSON.stringify(unavailable.body), /should-not-be-served/);
    } finally {
      await close(app);
    }
  });
...
test('static files and Beamer allowlist are safe', async () => {
  await withFixture(async ({ root, publicDir, templateDir }) => {
    const app = createServer({ rootDir: root, publicDir, templateDir, env: {} });
    const base = await listen(app);
    try {
      ...
      const theme = await fetch(`${base}/templates/beamer/beamerthemeBrand.sty`);
...
test('AI generation sends a bounded prompt and normalises a fenced response', async () => {
  await withFixture(async ({ root, publicDir, templateDir }) => {
    ...
    const upstream = createHttpServer((req, res) => {
      ...
      req.on('end', () => {
        ...
        const slides = Array.from({ length: 4 }, (_, index) => ({
          ...
          title: '这是一个超过六十字符的标题这是一个超过六十字符的标题这是一个超过六十字符的标题',
          subtitle: '副标题',
          bullets: ['一', '二', '三', '四', '五', '六'],
          notes: 'n'.repeat(600),
          seconds: 10,
    ...
    const app = createServer({
      ...
      publicDir,
      templateDir,
      env: { AI_API_KEY: 'secret-test-key', AI_BASE_URL: upstreamBase, AI_MODEL: 'mock-model' },
    });
    const base = await listen(app);
    try {
      ...
      assert.equal(result.body.deck.slides.reduce((sum, slide) => sum + slide.seconds, 0), 180);
      assert.ok(result.body.deck.slides.every((slide) => slide.bullets.length <= 5));
      assert.ok(result.body.deck.slides.every((slide) => slide.title.length <= 60));
      assert.ok(result.body.deck.slides.every((slide) => slide.notes.length <= 500));
...
test('demo refine shortens locally and AI refine remains explicit', async () => {
  await withFixture(async ({ root, publicDir, templateDir }) => {
    const app = createServer({ rootDir: root, publicDir, templateDir, env: { AI_API_KEY: '' } });
    const base = await listen(app);
    const slide = {
      id: 'slide-2',
      type: 'bullets',
