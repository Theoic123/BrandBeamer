...
function asBytes(value) {
  ...
  throw new TypeError(
    "ZIP file data must be a string, Uint8Array, or ArrayBuffer",
  );
}
...
function normaliseZipEntries(files) {
  ...
  if (files instanceof Map) {
    entries = Array.from(files.entries()).map(([name, data]) => ({
      name,
      data,
    }));
...
export function buildZip(files) {
  ...
  for (const entry of entries) {
    const local = new Uint8Array(
      30 + entry.nameBytes.length + entry.data.length,
    );
...
export function parseLogoDataUrl(dataUrl, maxBytes = MAX_LOGO_BYTES) {
  ...
  const match = /^data:(image\/png|image\/jpeg);base64,([\s\S]*)$/i.exec(
    dataUrl.trim(),
  );
  if (!match) {
  ...
  if (payload.length > maximumPayload) {
    throw new Error(
      `Logo exceeds the ${Math.floor(maxBytes / 1024 / 1024)} MiB size limit`,
    );
  }
...
function bulletText(value) {
  ...
  if (typeof value === "object" && !Array.isArray(value)) {
    const heading = stringValue(
      firstValue(value, ["heading", "title", "label"]),
    );
    const body = stringValue(
      firstValue(value, ["body", "text", "description", "value"]),
    );
    if (heading && body) {
...
function renderFrameStart(title, subtitle) {
  ...
  const lines = [
    escapedTitle ? `\\begin{frame}{${escapedTitle}}` : "\\begin{frame}",
  ];
  if (escapedSubtitle) {
...
export function buildTex(deck = {}, brand = {}) {
  ...
  const documentTitle =
    safeDeck.title ||
    stringValue(firstCover && firstCover.title) ||
  ...
  const logoCommand = safeBrand.logo
    ? `\\brandsetlogo{${safeBrand.logo.filename}}`
    : "";
...
export async function downloadBeamer(deck = {}, brand = {}) {
  ...
  if (!themeResponse.ok) {
    throw new Error(
      `Unable to load beamerthemeBrand.sty (HTTP ${themeResponse.status})`,
    );
  }
  if (!licenseResponse.ok) {
    throw new Error(
      `Unable to load the Beamer license (HTTP ${licenseResponse.status})`,
    );
  }
