...
function parseDotEnv(contents) {
  const values = {};
  for (const originalLine of String(contents).replace(/^\uFEFF/, '').split(/\r?\n/)) {
 ...
 */
export function loadEnv({ envFile = join(process.cwd(), '.env'), overrides = {} } = {}) {
  return {
    ...readDotEnv(envFile),
    ...process.env,
    ...overrides,
...
function getRuntimeConfig(options = {}) {
  const envFile = options.envFile ?? join(options.rootDir ?? MODULE_DIR, '.env');
  ...
  return {
    ...
    aiTimeoutMs: Number.isFinite(Number(options.aiTimeoutMs)) ? Number(options.aiTimeoutMs) : DEFAULT_AI_TIMEOUT_MS,
...
function validateGenerateInput(input) {
  ...
  const material = validateString(input.material, 'material', { max: MAX_MATERIAL_LENGTH });
  const title = validateString(input.title, 'title', { max: MAX_INPUT_TITLE_LENGTH });
  ...
  for (const key of ['institution', 'department', 'presenter', 'color']) {
    brand[key] = validateString(input.brand[key], `brand.${key}`, { required: false, max: 200 });
  }
...
function validateSlideInput(slide) {
  ...
  const title = validateString(slide.title, 'slide.title', { max: MAX_INPUT_TITLE_LENGTH });
  const subtitle = slide.subtitle === undefined ? '' : validateString(slide.subtitle, 'slide.subtitle', { required: false, max: MAX_INPUT_SUBTITLE_LENGTH });
  const bullets = slide.bullets === undefined ? [] : slide.bullets;
  if (!Array.isArray(bullets)) throw new HttpError(400, 'slide.bullets 必须是数组');
  const safeBullets = bullets.slice(0, 5).map((bullet, index) => validateString(bullet, `slide.bullets[${index}]`, { max: MAX_INPUT_BULLET_LENGTH }));
  const notes = slide.notes === undefined ? '' : validateString(slide.notes, 'slide.notes', { required: false, max: MAX_INPUT_NOTES_LENGTH });
  const seconds = slide.seconds === undefined ? 30 : Number(slide.seconds);
  if (!Number.isFinite(seconds) || seconds <= 0) throw new HttpError(400, 'slide.seconds 必须是正数');
  const id = slide.id === undefined ? 'slide-1' : validateString(slide.id, 'slide.id', { max: 100 });
  return {
    id,
    type: slide.type,
    title,
...
function normaliseModelSlide(slide, index) {
  if (!isRecord(slide)) throw new ModelResponseError(`第 ${index + 1} 页不是对象`);
  if (!SLIDE_TYPES.has(slide.type)) throw new ModelResponseError(`第 ${index + 1} 页 type 无效`);
  if (!isNonEmptyString(slide.title)) throw new ModelResponseError(`第 ${index + 1} 页缺少 title`);
  ...
  const bullets = (slide.bullets ?? []).slice(0, 5).map((bullet) => {
    if (typeof bullet !== 'string') throw new ModelResponseError(`第 ${index + 1} 页 bullets 必须是字符串数组`);
    return clipText(bullet, MAX_OUTPUT_BULLET_LENGTH);
  });
  const seconds = Number(slide.seconds);
  return {
    id: isNonEmptyString(slide.id) ? clipText(slide.id, 100) : `slide-${index + 1}`,
    type: slide.type,
    title: clipText(slide.title, MAX_SHORT_TEXT),
    subtitle: typeof slide.subtitle === 'string' ? clipText(slide.subtitle, MAX_OUTPUT_SUBTITLE_LENGTH) : '',
    bullets,
    notes: typeof slide.notes === 'string' ? clipText(slide.notes, MAX_OUTPUT_NOTES_LENGTH) : '',
    seconds: Number.isFinite(seconds) && seconds > 0 ? Math.max(1, Math.round(seconds)) : 1,
  };
}

function fitSeconds(slides, targetSeconds) {
  if (!slides.length) return slides;
  const weights = slides.map((slide) => (Number.isFinite(slide.seconds) && slide.seconds > 0 ? slide.seconds : 1));
  const totalWeight = weights.reduce((sum, value) => sum + value, 0) || slides.length;
  const result = weights.map((weight) => Math.max(1, Math.floor((weight / totalWeight) * targetSeconds)));
  let difference = targetSeconds - result.reduce((sum, value) => sum + value, 0);
  ...
  if (difference > 0) {
    ...
    while (difference < 0) {
      const index = result.reduce((best, value, current) => value > result[best] ? current : best, 0);
...
function normaliseModelDeck(raw, input) {
  ...
  for (let index = 0; index < slides.length; index += 1) {
    if (!slides[index].id || seenIds.has(slides[index].id)) slides[index].id = `slide-${index + 1}`;
    seenIds.add(slides[index].id);
  }

  const title = isNonEmptyString(deck.title) ? clipText(deck.title, MAX_SHORT_TEXT) : clipText(input.title, MAX_SHORT_TEXT);
  return {
...
function splitMaterial(material) {
  ...
  const chunks = [];
  for (let index = 0; index < normalised.length; index += 100) chunks.push(normalised.slice(index, index + 100));
  return chunks.length ? chunks : ['材料未提供'];
}

function demoDeck(input) {
  const pieces = splitMaterial(input.material);
  const source = (index, fallback) => clipText(pieces[index % pieces.length] || fallback, 180);
  const slides = [
    {
      id: 'slide-1',
      type: 'cover',
    ...
    },
    {
      ...
      title: '重点摘录',
      subtitle: '按原文拆分展示',
      bullets: [source(1, '材料未提供'), source(2, '材料未提供'), source(3, '材料未提供')],
...
function demoRefine(slide, instruction) {
  const aggressive = /(短|精简|缩短|简洁|short|brief|concise)/iu.test(instruction);
  ...
  return {
    ...
    bullets: slide.bullets.slice(0, 5).map((bullet) => shortenForDemo(bullet, maxBullet)),
...
async function requestAi({ config, messages }) {
  ...
  if (typeof config.fetchImpl !== 'function') throw new HttpError(503, '当前运行环境不支持 AI 请求');
  ...
  try {
    ...
    try {
      ...
      if (error?.name === 'AbortError' || controller.signal.aborted) throw new HttpError(504, 'AI 服务响应超时');
      throw new HttpError(502, 'AI 服务请求失败');
    }

    let responseText = '';
    try {
      responseText = typeof response.text === 'function' ? await response.text() : JSON.stringify(await response.json());
    } catch (error) {
      if (error?.name === 'AbortError' || controller.signal.aborted) throw new HttpError(504, 'AI 服务响应超时');
      throw new HttpError(502, 'AI 服务返回内容无法读取');
    }
    const responseOk = response.ok === undefined
      ? !(Number.isFinite(Number(response.status)) && Number(response.status) >= 400)
      : response.ok;
    if (!responseOk) throw new HttpError(502, `AI 服务返回错误（HTTP ${response.status}）`);
...
function responseContent(payload) {
  ...
  if (typeof content !== 'string') throw new ModelResponseError('AI 返回中缺少文本内容');
  return content;
}
...
async function generate(input, config) {
  ...
  try {
    const payload = await requestAi({ config, messages: generationMessages(input) });
    const modelText = redactSecret(responseContent(payload), valueFromEnv(config.env, 'AI_API_KEY'));
    const modelJson = parseModelJson(modelText);
    return { deck: normaliseModelDeck(modelJson, input), mode: 'ai' };
  } catch (error) {
    if (error instanceof ModelResponseError) throw new HttpError(502, `AI 返回格式无效：${error.message}`);
    throw error;
  }
}
...
async function refine(input, config) {
  ...
  if (!['ai', 'demo'].includes(mode)) throw new HttpError(400, 'mode 必须是 ai 或 demo');
  if (mode === 'demo') return { slide: demoRefine(slide, instruction) };

  try {
    const payload = await requestAi({ config, messages: refineMessages(slide, instruction) });
    const modelText = redactSecret(responseContent(payload), valueFromEnv(config.env, 'AI_API_KEY'));
    const modelJson = parseModelJson(modelText);
...
async function safeStaticFile({ pathname, publicDir, templateDir }) {
  const templateRelative = {
    '/templates/beamer/beamerthemeBrand.sty': join(templateDir, 'beamer', 'beamerthemeBrand.sty'),
  ...
  const target = templateTarget ?? resolve(publicDir, `.${pathname === '/' ? '/index.html' : pathname}`);
  ...
  const contentType = pathname === '/templates/beamer/LICENSE' ? 'text/plain; charset=utf-8' : MIME_TYPES[extname(resolvedTarget).toLowerCase()] ?? 'application/octet-stream';
...
export function createServer(options = {}) {
  ...
  const templateDir = resolve(options.templateDir ?? join(rootDir, 'templates'));
  const config = getRuntimeConfig({ ...options, rootDir });
  const maxBodyBytes = Number.isFinite(Number(options.maxBodyBytes)) ? Number(options.maxBodyBytes) : MAX_JSON_BYTES;
  ...
  const server = createHttpServer(async (req, res) => {
    try {
      ...
      if (pathname === '/api/generate') {
        ...
        const body = validateGenerateInput(await parseJsonRequest(req, maxBodyBytes));
      ...
      if (req.method !== 'GET' && req.method !== 'HEAD') {
        jsonResponse(res, 405, { error: '请求方法不被支持' }, { allow: 'GET, HEAD' });
        return;
      }
  ...
  server.requestTimeout = Number.isFinite(Number(options.requestTimeoutMs)) ? Number(options.requestTimeoutMs) : 30_000;
  server.headersTimeout = Math.max(1_000, Math.min(server.requestTimeout, 30_000));
  return server;
}
...
if (isMainModule()) {
  ...
  server.listen(Number.isFinite(port) ? port : DEFAULT_PORT, host, () => {
    ...
    const display = typeof address === 'object' && address ? `${address.address}:${address.port}` : `${host}:${port}`;
